...
```mooncram
$ run_moon_ide moon ide find-references K --loc 'src/types.mbt:17:20'
Found 2 references for symbol 'K':
<WORKDIR>/src/types.mbt:17:20-17:21:
   |  (escaped)
   | ///| (escaped)
   | /// 桶结构，包含键值对信息和链表指针 (escaped)
17 | priv struct Bucket[K, V] { (escaped)
...
<WORKDIR>/src/types.mbt:19:9-19:10:
   | /// 桶结构，包含键值对信息和链表指针 (escaped)
...
```mooncram
$ run_moon_ide moon ide find-references 'Bucket'
...
```mooncram
$ run_moon_ide moon ide find-references 'IndexMapCore'
...
```mooncram
$ run_moon_ide moon ide find-references 'T'
...
```mooncram
$ run_moon_ide moon ide find-references 'Default_init_capacity'
...
```mooncram
$ run_moon_ide moon ide find-references 'calc_grow_threshold'
...
```mooncram
$ run_moon_ide moon ide find-references 'new'
