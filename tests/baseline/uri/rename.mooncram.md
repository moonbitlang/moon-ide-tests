...
```mooncram
$ run_moon_ide moon ide rename 'authority' 'authority_renamed' --loc 'uri.mbt:56:3'
*** Begin Patch
*** Update File: <WORKDIR>/uri.mbt
@@
 /// - fragment: reference to a secondary resource
...
@@
 ...
 pub fn empty() -> Uri {
-  { scheme: None, authority: None, path: "", query: None, fragment: None }
+  { scheme: None, authority_renamed: None, path: "", query: None, fragment: None }
 }
...
+      uri = { ..uri, authority_renamed: Some(auth) }
     }
...
+  match self.authority_renamed {
     Some(auth) => { (escaped)
       match auth.userinfo {
         Some(userinfo) => result = result + userinfo + "@" (escaped)
@@
 ///
...
+      let new_authority = match self.authority_renamed {
         Some(auth) => Some({ ..auth, host, }) (escaped)
         None => Some({ userinfo: None, host, port: None }) (escaped)
       }
-      { ..self, authority: new_authority }
+      { ..self, authority_renamed: new_authority }
     }
-    None => { ..self, authority: None } (escaped)
+    None => { ..self, authority_renamed: None } (escaped)
   }
 }
...
+                normalized = { ..normalized, authority_renamed: Some(new_auth) }
               }
             None => () (escaped)
           }
@@
...
-  match relative.authority {
+  match relative.authority_renamed {
     Some(_) => (escaped)
       resolved = {
         scheme: base.scheme,
-        authority: relative.authority,
+        authority_renamed: relative.authority_renamed,
         path: relative.path,
         query: relative.query,
...
@@
         ...
         resolved = {
           scheme: base.scheme,
-          authority: base.authority,
+          authority_renamed: base.authority_renamed,
           path: base.path,
           query: match relative.query {
             Some(_) => relative.query (escaped)
@@
...
```mooncram
$ run_moon_ide moon ide rename 'Authority' 'AuthorityRenamed' --loc 'uri.mbt:65:12'
*** Begin Patch
*** Update File: <WORKDIR>/uri.mbt
@@
 /// - fragment: reference to a secondary resource
...
+fn parse_authority(auth_str : String) -> AuthorityRenamed raise UriError {
...
```mooncram
$ run_moon_ide moon ide rename 'UriError' 'UriErrorRenamed' --loc 'uri.mbt:73:14'
*** Begin Patch
*** Update File: <WORKDIR>/uri.mbt
@@
...
@@
 ...
 /// - Other `UriError` variants for various parsing failures
-pub fn parse(uri_str : String) -> Uri raise UriError {
+pub fn parse(uri_str : String) -> Uri raise UriErrorRenamed {
   if uri_str.length() == 0 {
     raise EmptyUri
   }
@@
 ...
 /// Parse URI with known scheme
-fn parse_with_scheme(scheme : String, rest : String) -> Uri raise UriError {
+fn parse_with_scheme(scheme : String, rest : String) -> Uri raise UriErrorRenamed {
   let mut uri = empty()
   uri = { ..uri, scheme: Some(scheme) }
...
@@
 ...
 /// Parse authority component
-fn parse_authority(auth_str : String) -> Authority raise UriError {
+fn parse_authority(auth_str : String) -> Authority raise UriErrorRenamed {
   let mut userinfo : String? = None
   let mut port : Int? = None
   let mut remaining = auth_str
@@
 ...
 /// - RFC3986 Section 5.2 - Reference Resolution
-pub fn resolve(base : Uri, relative : Uri) -> Uri raise UriError {
+pub fn resolve(base : Uri, relative : Uri) -> Uri raise UriErrorRenamed {
   // If relative URI is absolute, return it as-is
   if relative.is_absolute() {
     return relative
*** Update File: <WORKDIR>/uri_test.mbt
...
-    @uri.UriError::EmptyUri => () // Expected (escaped)
+    @uri.UriErrorRenamed::EmptyUri => () // Expected (escaped)
...
-    @uri.UriError::InvalidScheme(scheme) => (escaped)
+    @uri.UriErrorRenamed::InvalidScheme(scheme) => (escaped)
...
-    @uri.UriError::InvalidPort(port) => (escaped)
+    @uri.UriErrorRenamed::InvalidPort(port) => (escaped)
       if port != "abc" {
...
-    @uri.UriError::InvalidPort(_) => () // Expected (escaped)
+    @uri.UriErrorRenamed::InvalidPort(_) => () // Expected (escaped)
...
-    @uri.UriError::InvalidPath(_) => () // Expected (escaped)
+    @uri.UriErrorRenamed::InvalidPath(_) => () // Expected (escaped)
...
-    @uri.UriError::InvalidQuery(_) => () // Expected (escaped)
+    @uri.UriErrorRenamed::InvalidQuery(_) => () // Expected (escaped)
...
-    @uri.UriError::InvalidFragment(_) => () // Expected (escaped)
+    @uri.UriErrorRenamed::InvalidFragment(_) => () // Expected (escaped)
...
```mooncram
$ run_moon_ide moon ide rename 'empty' 'empty_renamed' --loc 'uri.mbt:97:8'
*** Begin Patch
*** Update File: <WORKDIR>/README.mbt.md
@@
...
+  let built_uri = @uri.empty_renamed()
...
+  let empty_uri = @uri.empty_renamed()
...
-  let absolute_uri = @uri.empty().with_scheme(Some("https"))
+  let absolute_uri = @uri.empty_renamed().with_scheme(Some("https"))
-  let relative_uri = @uri.empty().with_path("/relative")
+  let relative_uri = @uri.empty_renamed().with_path("/relative")
 
   if !absolute_uri.is_absolute() {
...
+  let uri = empty_renamed().with_path_segments(["api", "v1", "users"])
   json_inspect(uri.path(), content="/api/v1/users")
 
   // Test empty segments
-  let empty_path_uri = empty().with_path_segments([])
+  let empty_path_uri = empty_renamed().with_path_segments([])
   json_inspect(empty_path_uri.path(), content="")
 
   // Test segments with special characters
-  let special_uri = empty().with_path_segments([
+  let special_uri = empty_renamed().with_path_segments([
     "path with spaces", "file+name", "resource@id",
   ])
   json_inspect(
@@
   )
...
+  let single_uri = empty_renamed().with_path_segments(["resource"])
   json_inspect(single_uri.path(), content="/resource")
 }
...
+  let uri_user_only = empty_renamed()
     .with_host(Some("example.com"))
...
+  let long_uri = empty_renamed().with_path_segments([long_segment])
   let segments = long_uri.path_segments()
   json_inspect(segments[0].length(), content=1000)
 
   // Test empty components
-  let empty_components_uri = empty()
+  let empty_components_uri = empty_renamed()
     .with_scheme(Some(""))
     .with_host(Some(""))
     .with_path("")
*** End Patch
...
```mooncram
$ run_moon_ide moon ide rename 'parse' 'parse_renamed' --loc 'uri.mbt:127:8'
*** Begin Patch
*** Update File: <WORKDIR>/README.mbt.md
@@
 ///|
...
```mooncram
$ run_moon_ide moon ide rename 'parse_with_scheme' 'parse_with_scheme_renamed' --loc 'uri.mbt:160:4'
*** Begin Patch
*** Update File: <WORKDIR>/uri.mbt
@@
...
+        return parse_with_scheme_renamed(scheme_part, rest)
       } else {
...
@@
 ...
 /// Parse URI with known scheme
-fn parse_with_scheme(scheme : String, rest : String) -> Uri raise UriError {
+fn parse_with_scheme_renamed(scheme : String, rest : String) -> Uri raise UriError {
   let mut uri = empty()
   uri = { ..uri, scheme: Some(scheme) }
