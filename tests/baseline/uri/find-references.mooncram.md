...
```mooncram
$ run_moon_ide moon ide find-references 'authority' --loc 'uri.mbt:56:3'
Found 28 references for symbol 'authority':
<WORKDIR>/uri.mbt:56:3-56:12:
   | /// - fragment: reference to a secondary resource (escaped)
   | pub struct Uri { (escaped)
...
200 |       uri = { ..uri, authority: Some(auth) } (escaped)
    |                      ^^^^^^^^^ (escaped)
    |     } (escaped)
    |  (escaped)

<WORKDIR>/uri.mbt:238:18-238:27:
    |   match self.scheme { (escaped)
    |     Some(scheme) => { (escaped)
    |       result = result + scheme (escaped)
238 |       match self.authority { (escaped)
    |                  ^^^^^^^^^ (escaped)
    |         Some(_) => result = result + "://" (escaped)
    |         None => result = result + ":" (escaped)

<WORKDIR>/uri.mbt:247:14-247:23:
    |   } (escaped)
    |  (escaped)
    |   // Add authority (escaped)
247 |   match self.authority { (escaped)
    |              ^^^^^^^^^ (escaped)
    |     Some(auth) => { (escaped)
    |       match auth.userinfo { (escaped)

<WORKDIR>/uri.mbt:346:14-346:23:
    | /// (escaped)
    | /// The host as `Some(String)` if present, `None` for relative URIs or URIs without authority. (escaped)
    | pub fn Uri::host(self : Uri) -> String? { (escaped)
346 |   match self.authority { (escaped)
    |              ^^^^^^^^^ (escaped)
    |     Some(auth) => Some(auth.host) (escaped)
    |     None => None (escaped)

<WORKDIR>/uri.mbt:386:14-386:23:
    | /// (escaped)
    | /// - `effective_port()` - Get the effective port including scheme defaults (escaped)
    | pub fn Uri::port(self : Uri) -> Int? { (escaped)
386 |   match self.authority { (escaped)
    |              ^^^^^^^^^ (escaped)
    |     Some(auth) => auth.port (escaped)
    |     None => None (escaped)

<WORKDIR>/uri.mbt:739:38-739:47:
    | pub fn Uri::with_host(self : Uri, new_host : String?) -> Uri { (escaped)
    |   match new_host { (escaped)
    |     Some(host) => { (escaped)
739 |       let new_authority = match self.authority { (escaped)
    |                                      ^^^^^^^^^ (escaped)
    |         Some(auth) => Some({ ..auth, host, }) (escaped)
    |         None => Some({ userinfo: None, host, port: None }) (escaped)

<WORKDIR>/uri.mbt:743:17-743:26:
    |         Some(auth) => Some({ ..auth, host, }) (escaped)
    |         None => Some({ userinfo: None, host, port: None }) (escaped)
    |       } (escaped)
743 |       { ..self, authority: new_authority } (escaped)
    |                 ^^^^^^^^^ (escaped)
    |     } (escaped)
    |     None => { ..self, authority: None } (escaped)

<WORKDIR>/uri.mbt:745:23-745:32:
    |       } (escaped)
    |       { ..self, authority: new_authority } (escaped)
    |     } (escaped)
745 |     None => { ..self, authority: None } (escaped)
    |                       ^^^^^^^^^ (escaped)
    |   } (escaped)
    | } (escaped)

<WORKDIR>/uri.mbt:752:14-752:23:
    | ///| (escaped)
    | /// Create a new URI with the specified port (escaped)
    | pub fn Uri::with_port(self : Uri, new_port : Int?) -> Uri { (escaped)
752 |   match self.authority { (escaped)
    |              ^^^^^^^^^ (escaped)
    |     Some(auth) => { (escaped)
    |       let new_authority = Some({ ..auth, port: new_port }) (escaped)

<WORKDIR>/uri.mbt:755:17-755:26:
    |   match self.authority { (escaped)
    |     Some(auth) => { (escaped)
    |       let new_authority = Some({ ..auth, port: new_port }) (escaped)
755 |       { ..self, authority: new_authority } (escaped)
    |                 ^^^^^^^^^ (escaped)
    |     } (escaped)
    |     None => (escaped)

<WORKDIR>/uri.mbt:765:21-765:30:
    |             host: "", (escaped)
    |             port: Some(port), (escaped)
    |           }) (escaped)
765 |           { ..self, authority: new_authority } (escaped)
    |                     ^^^^^^^^^ (escaped)
    |         } (escaped)
    |         None => self (escaped)

<WORKDIR>/uri.mbt:828:14-828:23:
    | ///| (escaped)
    | /// Get the effective port (explicit port or default port for scheme) (escaped)
    | pub fn Uri::effective_port(self : Uri) -> Int? { (escaped)
828 |   match self.authority { (escaped)
    |              ^^^^^^^^^ (escaped)
    |     Some(auth) => (escaped)
    |       match auth.port { (escaped)

<WORKDIR>/uri.mbt:848:14-848:23:
    |   let mut normalized = self (escaped)
    |  (escaped)
    |   // Remove default port if present (escaped)
848 |   match self.authority { (escaped)
    |              ^^^^^^^^^ (escaped)
    |     Some(auth) => (escaped)
    |       match (self.scheme, auth.port) { (escaped)

<WORKDIR>/uri.mbt:856:46-856:55:
    |             Some(default) => (escaped)
    |               if port == default { (escaped)
    |                 let new_auth = { ..auth, port: None } (escaped)
856 |                 normalized = { ..normalized, authority: Some(new_auth) } (escaped)
    |                                              ^^^^^^^^^ (escaped)
    |               } (escaped)
    |             None => () (escaped)

<WORKDIR>/uri.mbt:969:18-969:27:
    |   let mut resolved = base (escaped)
    |  (escaped)
    |   // If relative has authority, use it and its path/query/fragment (escaped)
969 |   match relative.authority { (escaped)
    |                  ^^^^^^^^^ (escaped)
    |     Some(_) => (escaped)
    |       resolved = { (escaped)

<WORKDIR>/uri.mbt:973:9-973:18:
    |     Some(_) => (escaped)
    |       resolved = { (escaped)
    |         scheme: base.scheme, (escaped)
973 |         authority: relative.authority, (escaped)
    |         ^^^^^^^^^ (escaped)
    |         path: relative.path, (escaped)
    |         query: relative.query, (escaped)

<WORKDIR>/uri.mbt:973:29-973:38:
    |     Some(_) => (escaped)
    |       resolved = { (escaped)
    |         scheme: base.scheme, (escaped)
973 |         authority: relative.authority, (escaped)
...
<WORKDIR>/uri.mbt:984:11-984:20:
    |         // Empty path, use base path (escaped)
    |         resolved = { (escaped)
    |           scheme: base.scheme, (escaped)
984 |           authority: base.authority, (escaped)
    |           ^^^^^^^^^ (escaped)
    |           path: base.path, (escaped)
    |           query: match relative.query { (escaped)

<WORKDIR>/uri.mbt:984:27-984:36:
    |         // Empty path, use base path (escaped)
    |         resolved = { (escaped)
    |           scheme: base.scheme, (escaped)
984 |           authority: base.authority, (escaped)
...
<WORKDIR>/uri.mbt:1004:11-1004:20:
     |  (escaped)
     |         resolved = { (escaped)
     |           scheme: base.scheme, (escaped)
1004 |           authority: base.authority, (escaped)
     |           ^^^^^^^^^ (escaped)
     |           path: resolved_path, (escaped)
     |           query: relative.query, (escaped)

<WORKDIR>/uri.mbt:1004:27-1004:36:
     |  (escaped)
     |         resolved = { (escaped)
     |           scheme: base.scheme, (escaped)
1004 |           authority: base.authority, (escaped)
...
<WORKDIR>/uri.mbt:1471:14-1471:23:
     | /// (escaped)
     | /// Optional tuple of (username, optional password) (escaped)
     | pub fn Uri::userinfo_components(self : Uri) -> (String, String?)? { (escaped)
1471 |   match self.authority { (escaped)
     |              ^^^^^^^^^ (escaped)
     |     Some(auth) => (escaped)
     |       match auth.userinfo { (escaped)

<WORKDIR>/uri.mbt:1527:38-1527:47:
     |         None => Uri::encode(user) (escaped)
     |       } (escaped)
     |  (escaped)
1527 |       let new_authority = match self.authority { (escaped)
     |                                      ^^^^^^^^^ (escaped)
     |         Some(auth) => Some({ ..auth, userinfo: Some(userinfo) }) (escaped)
     |         None => Some({ userinfo: Some(userinfo), host: "", port: None }) (escaped)

<WORKDIR>/uri.mbt:1532:17-1532:26:
     |         None => Some({ userinfo: Some(userinfo), host: "", port: None }) (escaped)
     |       } (escaped)
     |  (escaped)
1532 |       { ..self, authority: new_authority } (escaped)
     |                 ^^^^^^^^^ (escaped)
     |     } (escaped)
     |     None => (escaped)

<WORKDIR>/uri.mbt:1535:18-1535:27:
     |       { ..self, authority: new_authority } (escaped)
     |     } (escaped)
     |     None => (escaped)
1535 |       match self.authority { (escaped)
     |                  ^^^^^^^^^ (escaped)
     |         Some(auth) => { (escaped)
     |           let new_authority = Some({ ..auth, userinfo: None }) (escaped)

<WORKDIR>/uri.mbt:1538:21-1538:30:
     |       match self.authority { (escaped)
     |         Some(auth) => { (escaped)
     |           let new_authority = Some({ ..auth, userinfo: None }) (escaped)
1538 |           { ..self, authority: new_authority } (escaped)
     |                     ^^^^^^^^^ (escaped)
     |         } (escaped)
     |         None => self (escaped)
...
```mooncram
$ run_moon_ide moon ide find-references 'Authority' --loc 'uri.mbt:65:12'
...
```mooncram
$ run_moon_ide moon ide find-references 'UriError' --loc 'uri.mbt:73:14'
...
```mooncram
$ run_moon_ide moon ide find-references 'empty' --loc 'uri.mbt:97:8'
Found 21 references for symbol 'empty':
<WORKDIR>/README.mbt.md:45:24-45:29:
   |   debug_inspect(uri.fragment(), content="Some(\\"fragment\\")") (escaped)
   |  (escaped)
   |   // Build a URI using the builder methods (escaped)
45 |   let built_uri = @uri.empty() (escaped)
   |                        ^^^^^ (escaped)
   |     .with_scheme(Some("https")) (escaped)
   |     .with_host(Some("api.example.com")) (escaped)

<WORKDIR>/README.mbt.md:219:22-219:27:
    | ```moonbit check (escaped)
    | ///| (escaped)
    | test "build_api_uri" { (escaped)
219 |   let api_uri = @uri.empty() (escaped)
    |                      ^^^^^ (escaped)
    |     .with_scheme(Some("https")) (escaped)
    |     .with_host(Some("api.github.com")) (escaped)

<WORKDIR>/uri.mbt:97:8-97:13:
   | /// # Returns (escaped)
   | /// (escaped)
   | /// A new `Uri` with all components set to their default values. (escaped)
97 | pub fn empty() -> Uri { (escaped)
...
<WORKDIR>/uri.mbt:133:13-133:18:
    |   } (escaped)
    |  (escaped)
    |   // Start with an empty URI and parse components (escaped)
133 |   let uri = empty() (escaped)
    |             ^^^^^ (escaped)
    |   let remaining = uri_str (escaped)
    |  (escaped)

<WORKDIR>/uri.mbt:161:17-161:22:
    | ///| (escaped)
    | /// Parse URI with known scheme (escaped)
    | fn parse_with_scheme(scheme : String, rest : String) -> Uri raise UriError { (escaped)
161 |   let mut uri = empty() (escaped)
    |                 ^^^^^ (escaped)
    |   uri = { ..uri, scheme: Some(scheme) } (escaped)
    |  (escaped)

<WORKDIR>/uri_test.mbt:16:24-16:29:
   | ///| (escaped)
   | /// Test empty URI (escaped)
   | test "empty_uri" { (escaped)
16 |   let empty_uri = @uri.empty() (escaped)
   |                        ^^^^^ (escaped)
   |   json_inspect(empty_uri, content={ "path": "" }) (escaped)
   | } (escaped)

<WORKDIR>/uri_test.mbt:23:18-23:23:
   | ///| (escaped)
   | /// Test URI building (escaped)
   | test "uri_building" { (escaped)
23 |   let uri = @uri.empty() (escaped)
   |                  ^^^^^ (escaped)
   |     .with_scheme(Some("https")) (escaped)
   |     .with_host(Some("api.example.com")) (escaped)

<WORKDIR>/uri_test.mbt:40:18-40:23:
   | ///| (escaped)
   | /// Test URI serialization (escaped)
   | test "uri_serialization" { (escaped)
40 |   let uri = @uri.empty() (escaped)
   |                  ^^^^^ (escaped)
   |     .with_scheme(Some("http")) (escaped)
   |     .with_host(Some("localhost")) (escaped)

<WORKDIR>/uri_test.mbt:91:27-91:32:
   | ///| (escaped)
   | /// Test URI absolute/relative checks (escaped)
   | test "uri_absolute_relative" { (escaped)
91 |   let absolute_uri = @uri.empty().with_scheme(Some("https")) (escaped)
   |                           ^^^^^ (escaped)
   |   let relative_uri = @uri.empty().with_path("/relative") (escaped)
   |  (escaped)

<WORKDIR>/uri_test.mbt:92:27-92:32:
   | /// Test URI absolute/relative checks (escaped)
   | test "uri_absolute_relative" { (escaped)
   |   let absolute_uri = @uri.empty().with_scheme(Some("https")) (escaped)
92 |   let relative_uri = @uri.empty().with_path("/relative") (escaped)
   |                           ^^^^^ (escaped)
   |  (escaped)
   |   if !absolute_uri.is_absolute() { (escaped)

<WORKDIR>/uri_test_new_features.mbt:113:19-113:24:
    |   json_inspect(segments, content=["path", "to", "resource"]) (escaped)
    |  (escaped)
    |   // Test empty path (escaped)
113 |   let empty_uri = empty() (escaped)
    |                   ^^^^^ (escaped)
    |   let empty_segments = empty_uri.path_segments() (escaped)
    |   json_inspect(empty_segments.length(), content=0) (escaped)

<WORKDIR>/uri_test_new_features.mbt:132:13-132:18:
    | /// Test path segments building (escaped)
    | test "path_segments_building" { (escaped)
    |   // Test basic path building (escaped)
132 |   let uri = empty().with_path_segments(["api", "v1", "users"]) (escaped)
    |             ^^^^^ (escaped)
    |   json_inspect(uri.path(), content="/api/v1/users") (escaped)
    |  (escaped)

<WORKDIR>/uri_test_new_features.mbt:136:24-136:29:
    |   json_inspect(uri.path(), content="/api/v1/users") (escaped)
    |  (escaped)
    |   // Test empty segments (escaped)
136 |   let empty_path_uri = empty().with_path_segments([]) (escaped)
    |                        ^^^^^ (escaped)
    |   json_inspect(empty_path_uri.path(), content="") (escaped)
    |  (escaped)

<WORKDIR>/uri_test_new_features.mbt:140:21-140:26:
    |   json_inspect(empty_path_uri.path(), content="") (escaped)
    |  (escaped)
    |   // Test segments with special characters (escaped)
140 |   let special_uri = empty().with_path_segments([ (escaped)
    |                     ^^^^^ (escaped)
    |     "path with spaces", "file+name", "resource@id", (escaped)
    |   ]) (escaped)

<WORKDIR>/uri_test_new_features.mbt:149:20-149:25:
    |   ) (escaped)
    |  (escaped)
    |   // Test single segment (escaped)
149 |   let single_uri = empty().with_path_segments(["resource"]) (escaped)
    |                    ^^^^^ (escaped)
    |   json_inspect(single_uri.path(), content="/resource") (escaped)
    | } (escaped)

<WORKDIR>/uri_test_new_features.mbt:195:13-195:18:
    | /// Test userinfo building (escaped)
    | test "userinfo_building" { (escaped)
    |   // Test setting username and password (escaped)
195 |   let uri = empty() (escaped)
    |             ^^^^^ (escaped)
    |     .with_host(Some("example.com")) (escaped)
    |     .with_userinfo(Some("user"), Some("pass")) (escaped)

<WORKDIR>/uri_test_new_features.mbt:208:23-208:28:
    |   } (escaped)
    |  (escaped)
    |   // Test setting username only (escaped)
208 |   let uri_user_only = empty() (escaped)
    |                       ^^^^^ (escaped)
    |     .with_host(Some("example.com")) (escaped)
    |     .with_userinfo(Some("user"), None) (escaped)

<WORKDIR>/uri_test_new_features.mbt:225:21-225:26:
    |   } (escaped)
    |  (escaped)
    |   // Test special characters in userinfo (escaped)
225 |   let uri_special = empty() (escaped)
    |                     ^^^^^ (escaped)
    |     .with_host(Some("example.com")) (escaped)
    |     .with_userinfo(Some("user@domain"), Some("pass word")) (escaped)

<WORKDIR>/uri_test_new_features.mbt:238:13-238:18:
    | ///| (escaped)
    | /// Test comprehensive URI building with all components (escaped)
    | test "comprehensive_uri_building" { (escaped)
238 |   let uri = empty() (escaped)
    |             ^^^^^ (escaped)
    |     .with_scheme(Some("https")) (escaped)
    |     .with_userinfo(Some("admin"), Some("secret")) (escaped)

<WORKDIR>/uri_test_new_features.mbt:274:18-274:23:
    |  (escaped)
    |   // Test very long URI components (escaped)
    |   let long_segment = "a".repeat(1000) (escaped)
274 |   let long_uri = empty().with_path_segments([long_segment]) (escaped)
    |                  ^^^^^ (escaped)
    |   let segments = long_uri.path_segments() (escaped)
    |   json_inspect(segments[0].length(), content=1000) (escaped)

<WORKDIR>/uri_test_new_features.mbt:279:30-279:35:
    |   json_inspect(segments[0].length(), content=1000) (escaped)
    |  (escaped)
    |   // Test empty components (escaped)
279 |   let empty_components_uri = empty() (escaped)
    |                              ^^^^^ (escaped)
    |     .with_scheme(Some("")) (escaped)
    |     .with_host(Some("")) (escaped)

```

```mooncram
$ run_moon_ide moon ide find-references 'parse' --loc 'uri.mbt:127:8'
Found 47 references for symbol 'parse':
<WORKDIR>/README.mbt.md:36:18-36:23:
   | ///| (escaped)
   | test "quick_start_example" { (escaped)
...
```mooncram
$ run_moon_ide moon ide find-references 'parse_with_scheme' --loc 'uri.mbt:160:4'
