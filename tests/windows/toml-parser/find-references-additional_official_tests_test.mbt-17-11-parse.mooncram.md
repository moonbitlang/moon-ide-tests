# toml-parser find-references parse additional_official_tests_test.mbt:17:11

```mooncram
$ run_moon_ide '..\..\..\fixtures\repos\toml-parser' moon ide find-references 'parse' --loc 'additional_official_tests_test.mbt:17:11'
Found 191 references for symbol 'parse':
<WORKDIR>/README.mbt.md:112:22-112:27:
    | ///|
    | test {
    |   // Quick start example - parsing and accessing values
112 |   let config = @toml.parse(
    |                      ^^^^^
    |     (
    |       #|title = "My App"

<WORKDIR>/README.mbt.md:233:22-233:27:
    | test {
    |   // Error handling with try-catch
    |   let invalid_toml = "invalid = [unclosed"
233 |   let config = @toml.parse(invalid_toml) catch {
    |                      ^^^^^
    |     _ => TomlTable(Map([])) // Return default value on error
    |   }

<WORKDIR>/README.mbt.md:240:26-240:31:
    |   assert_eq(table.length(), 0) // Empty table from error handler
    | 
    |   // Error handling with try? - converts to Result type
240 |   let result = try @toml.parse("key = \"value\"") catch {
    |                          ^^^^^
    |     err => Err(err)
    |   } noraise {

<WORKDIR>/README.mbt.md:250:30-250:35:
    |   }
    | 
    |   // Parsing error example
250 |   let bad_result = try @toml.parse("bad syntax here") catch {
    |                              ^^^^^
    |     err => Err(err)
    |   } noraise {

<WORKDIR>/README.mbt.md:284:11-284:16:
    |     #|max_connections = 100
    |     #|
    |   debug_inspect(
284 |     @toml.parse(toml),
...
<WORKDIR>/README.mbt.md:311:11-311:16:
    |     #|booleans = [true, false, true]
    |     #|
    |   debug_inspect(
311 |     @toml.parse(toml_arrays),
    |           ^^^^^
    |     content=(
    |       #|TomlTable(

<WORKDIR>/README.mbt.md:355:11-355:16:
    |     #|sku = 284758393
    |     #|
    |   debug_inspect(
355 |     @toml.parse(toml_tables),
    |           ^^^^^
    |     content=(
    |       #|TomlTable(

<WORKDIR>/README.mbt.md:387:11-387:16:
    |     #|meeting_time = 14:30:00
    |     #|
    |   debug_inspect(
387 |     @toml.parse(toml_datetime),
    |           ^^^^^
    |     content=(
    |       #|TomlTable(

<WORKDIR>/README.mbt.md:415:11-415:16:
    |     #|cache = {enabled = true, ttl = 300}
    |     #|
    |   debug_inspect(
415 |     @toml.parse(toml_inline),
    |           ^^^^^
    |     content=(
    |       #|TomlTable(

<WORKDIR>/README.mbt.md:453:22-453:27:
    |     #|  2023-08-20T02:00:00
    |     #|]
    |     #|
453 |   let result = @toml.parse(config)
    |                      ^^^^^
    |   assert_true(result.validate())
    |   // Verify the structure contains expected keys

<WORKDIR>/README.mbt.md:505:11-505:16:
    |     #|weight = 0.5
    |     #|
    |   debug_inspect(
505 |     @toml.parse(toml_advanced),
    |           ^^^^^
    |     content=(
    |       #|TomlTable(

<WORKDIR>/README.mbt.md:598:27-598:32:
    | ///|
    | test {
    |   // Working with nested tables
598 |   let parsed_toml = @toml.parse(
    |                           ^^^^^
    |     (
    |       #|[database]

<WORKDIR>/README.mbt.md:638:21-638:26:
    | ///|
    | test {
    |   // Valid: all integers
638 |   let valid = @toml.parse("numbers = [1, 2, 3]")
    |                     ^^^^^
    |   assert_true(valid.validate())
    | 

<WORKDIR>/README.mbt.md:642:21-642:26:
    |   assert_true(valid.validate())
    | 
    |   // Mixed types are allowed during parsing but fail validation
642 |   let mixed = @toml.parse("mixed = [1, \"two\", 3.0]")
    |                     ^^^^^
    |   assert_false(mixed.validate()) // Validation catches the type mismatch
    | }

<WORKDIR>/README.mbt.md:653:27-653:32:
    | ///|
    | test {
    |   // Working with all 4 datetime types
653 |   let parsed_toml = @toml.parse(
    |                           ^^^^^
    |     (
...
<WORKDIR>/toml_cli\main.mbt:105:25-105:30:
    |       return 1
    |     }
    |   }
105 |   let value = @toml_lib.parse(source) catch {
    |                         ^^^^^
    |     err => {
    |       println("error: failed to parse \{path}: \{err}")

<WORKDIR>/toml_cli\main.mbt:123:21-123:26:
    |       return 1
    |     }
    |   }
123 |   let _ = @toml_lib.parse(source) catch {
    |                     ^^^^^
    |     err => {
    |       println("error: failed to parse \{path}: \{err}")

<WORKDIR>/toml_cli\tojson.mbt:38:25-38:30:
   |       return 1
   |     }
   |   }
38 |   let value = @toml_lib.parse(source) catch {
   |                         ^^^^^
   |     err => {
   |       println("error: failed to parse \{path}: \{err}")
