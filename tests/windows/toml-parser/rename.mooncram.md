...
```mooncram
$ run_moon_ide '..\..\..\fixtures\repos\toml-parser' moon ide rename 'TestResult' 'TestResultRenamed' --loc 'e2e\runner.mbt:7:12'
*** Begin Patch
...
```mooncram
$ run_moon_ide '..\..\..\fixtures\repos\toml-parser' moon ide rename 'passed' 'passed_renamed' --loc 'e2e\runner.mbt:8:3'
*** Begin Patch
*** Update File: <WORKDIR>/e2e\runner.mbt
@@
 /// fixture path and a short reason) so that the test harness can print a
...
```mooncram
$ run_moon_ide '..\..\..\fixtures\repos\toml-parser' moon ide rename 'to_test_json' 'to_test_json_renamed' --loc 'e2e\convert.mbt:5:8'
*** Begin Patch
...
+    TomlArray(arr) => Json::array([ for v in arr => to_test_json_renamed(v) ])
     TomlTable(table) =>
       Json::object(
-        Map::from_array([ for k, v in table => (k, to_test_json(v)) ]),
+        Map::from_array([ for k, v in table => (k, to_test_json_renamed(v)) ]),
       )
   }
 }
...
```mooncram
$ run_moon_ide '..\..\..\fixtures\repos\toml-parser' moon ide rename 'value' 'value_renamed' --loc 'e2e\convert.mbt:5:21'
*** Begin Patch
*** Update File: <WORKDIR>/e2e\convert.mbt
@@
 /// Convert a TomlValue to the toml-test JSON format.
...
```mooncram
$ run_moon_ide '..\..\..\fixtures\repos\toml-parser' moon ide rename 'files' 'files_renamed' --loc 'e2e\e2e_test.mbt:3:7'
*** Begin Patch
*** Update File: <WORKDIR>/e2e\e2e_test.mbt
@@
 ///|
...
```mooncram
$ run_moon_ide '..\..\..\fixtures\repos\toml-parser' moon ide rename 'collect_toml_files' 'collect_toml_files_renamed' --loc 'e2e\e2e_test.mbt:4:8'
*** Begin Patch
*** Update File: <WORKDIR>/e2e\e2e_test.mbt
@@
 ///|
...
```mooncram
$ run_moon_ide '..\..\..\fixtures\repos\toml-parser' moon ide rename 'result' 'result_renamed' --loc 'e2e\known_failures_test.mbt:10:7'
*** Begin Patch
...
```mooncram
$ run_moon_ide '..\..\..\fixtures\repos\toml-parser' moon ide rename 'parse' 'parse_renamed' --loc 'e2e\known_failures_test.mbt:10:26'
*** Begin Patch
