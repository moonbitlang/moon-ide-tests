...
```mooncram
$ run_moon_ide '..\..\..\fixtures\repos\toml-parser' moon ide find-references 'TestResult' --loc 'e2e\runner.mbt:7:12'
...
```mooncram
$ run_moon_ide '..\..\..\fixtures\repos\toml-parser' moon ide find-references 'passed' --loc 'e2e\runner.mbt:8:3'
...
```mooncram
$ run_moon_ide '..\..\..\fixtures\repos\toml-parser' moon ide find-references 'to_test_json' --loc 'e2e\convert.mbt:5:8'
...
<WORKDIR>/e2e\convert.mbt:20:53-20:65:
   ...
   |       }
   |     }
20 |     TomlArray(arr) => Json::array([ for v in arr => to_test_json(v) ])
   |                                                     ^^^^^^^^^^^^
   |     TomlTable(table) =>
   |       Json::object(

<WORKDIR>/e2e\convert.mbt:23:52-23:64:
   |     TomlArray(arr) => Json::array([ for v in arr => to_test_json(v) ])
   |     TomlTable(table) =>
   |       Json::object(
23 |         Map::from_array([ for k, v in table => (k, to_test_json(v)) ]),
   |                                                    ^^^^^^^^^^^^
   |       )
   |   }
...
```mooncram
$ run_moon_ide '..\..\..\fixtures\repos\toml-parser' moon ide find-references 'value' --loc 'e2e\convert.mbt:5:21'
...
<WORKDIR>/e2e\convert.mbt:12:23-12:28:
   ...
   |     TomlDateTime(_) => {
12 |       let (kind, s) = value.datetime_info().unwrap()
   |                       ^^^^^
   |       match kind {
...
```mooncram
$ run_moon_ide '..\..\..\fixtures\repos\toml-parser' moon ide find-references 'files' --loc 'e2e\e2e_test.mbt:3:7'
...
4 |   @e2e.collect_toml_files("e2e/testdata/valid", files)
  |                                                 ^^^^^
  |   files.sort()
  |   let mut passed = 0
...
```mooncram
$ run_moon_ide '..\..\..\fixtures\repos\toml-parser' moon ide find-references 'collect_toml_files' --loc 'e2e\e2e_test.mbt:4:8'
...
<WORKDIR>/e2e\runner.mbt:29:7-29:25:
   ...
   |     let path = "\{dir}/\{entry}"
   |     if @fs.is_dir(path) {
29 |       collect_toml_files(path, files)
   |       ^^^^^^^^^^^^^^^^^^
...
```mooncram
$ run_moon_ide '..\..\..\fixtures\repos\toml-parser' moon ide find-references 'result' --loc 'e2e\known_failures_test.mbt:10:7'
...
```mooncram
$ run_moon_ide '..\..\..\fixtures\repos\toml-parser' moon ide find-references 'parse' --loc 'e2e\known_failures_test.mbt:10:26'
...
<WORKDIR>/cmd\toml\main.mbt:76:25-76:30:
   |       return 1
   |     }
   |   }
...
<WORKDIR>/cmd\toml\main.mbt:94:21-94:26:
   |       return 1
   |     }
   |   }
