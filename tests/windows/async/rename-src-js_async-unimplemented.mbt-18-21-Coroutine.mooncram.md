...
```mooncram
$ run_moon_ide '..\..\..\fixtures\repos\async' moon ide rename 'Coroutine' 'CoroutineRenamed' --loc 'src\js_async\unimplemented.mbt:18:21'
*** Begin Patch
...
@@
 ...
 ///|
-pub fn Coroutine::unwrap(self : Coroutine) -> SuspendResult raise {
+pub fn CoroutineRenamed::unwrap(self : CoroutineRenamed) -> SuspendResult raise {
   match self.state {
     Done => Continue
     Cancelled => Cancelled
@@
 ...
 ///|
-pub async fn Coroutine::wait(target : Coroutine) -> SuspendResult {
+pub async fn CoroutineRenamed::wait(target : CoroutineRenamed) -> SuspendResult {
...
+  waiter : @coroutine.CoroutineRenamed
 }
...
+  mut coro : @coroutine.CoroutineRenamed?
 ...
 ///|
*** Update File: <WORKDIR>/src\task.mbt
@@
...
*** Update File: <WORKDIR>/src\timer.mbt
@@
