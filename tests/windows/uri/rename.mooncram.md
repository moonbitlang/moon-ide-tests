...
```mooncram
$ run_moon_ide '..\..\..\fixtures\repos\uri' moon ide rename 'Uri' 'UriRenamed' --loc 'uri.mbt:54:12'
*** Begin Patch
...
+        Some(pass) => UriRenamed::encode(user) + ":" + UriRenamed::encode(pass)
-        None => Uri::encode(user)
+        None => UriRenamed::encode(user)
       }
...
```mooncram
$ run_moon_ide '..\..\..\fixtures\repos\uri' moon ide rename 'scheme' 'scheme_renamed' --loc 'uri.mbt:55:3'
*** Begin Patch
*** Update File: <WORKDIR>/uri.mbt
@@
 /// - query: additional parameters
...
```mooncram
$ run_moon_ide '..\..\..\fixtures\repos\uri' moon ide rename 'parse' 'parse_renamed' --loc 'uri_test.mbt:5:18'
*** Begin Patch
...
+    let _ = @uri.parse_renamed("")
     ...
     @uri.UriError::EmptyUri => () // Expected
@@
 /// Test invalid scheme
 test "invalid_scheme" {
   try {
...
+    let _ = @uri.parse_renamed("123invalid://example.com")
     ...
     @uri.UriError::InvalidScheme(scheme) =>
@@
 /// Test invalid port
 test "invalid_port" {
   try {
...
+    let _ = @uri.parse_renamed("http://example.com:99999/path")
     ...
     @uri.UriError::InvalidPort(_) => () // Expected
@@
...
+    let _ = @uri.parse_renamed("http://example.com/path with spaces")
     ...
     @uri.UriError::InvalidPath(_) => () // Expected
@@
...
+    let _ = @uri.parse_renamed("http://example.com/path?query with spaces")
     ...
     @uri.UriError::InvalidQuery(_) => () // Expected
@@
...
+    let _ = @uri.parse_renamed("http://example.com/path#fragment with spaces")
     ...
     @uri.UriError::InvalidFragment(_) => () // Expected
@@
...
```mooncram
$ run_moon_ide '..\..\..\fixtures\repos\uri' moon ide rename 'Uri' 'UriRenamed' --loc 'uri_test_new_features.mbt:5:17'
*** Begin Patch
*** Update File: <WORKDIR>/uri.mbt
@@
 /// - path: hierarchical path to resource
