...
```mooncram
$ run_moon_ide moon ide peek-def 'new' --loc 'warren/templates/minimized/main.mbt:1:18'
Definition found at file <WORKDIR>/warren/templates/minimized/.mooncakes/moonbit-community/rabbita/top.mbt
   | ///   ignore(app)
   | ///   // use `@rabbita.new(app).mount("id")` in client
   | /// }
   | /// ```
   | #cfg(target="js")
33 | pub fn new(root : Cell) -> App {
   |        ^^^
...
```mooncram
$ run_moon_ide moon ide peek-def 'create_pure_state' --loc 'warren/templates/minimized/main.mbt:1:23'
Error: could not find definition for symbol 'create_pure_state' at warren/templates/minimized/main.mbt:1:23
[1]
```
