...
<WORKDIR>/internal/tokenize/tokenize.mbt:760:13-760:27:
    |           self.update_view(rest3)
    |           let end_pos = self.get_loc()
    |           DateTimeToken(
760 |             OffsetDateTime("\{datetime_str}Z"),
    |             ^^^^^^^^^^^^^^
    |             loc=make_loc(start_pos, end_pos),
    |           )

<WORKDIR>/internal/tokenize/tokenize.mbt:771:13-771:27:
    |           validate_timezone_offset(offset)
    |           let end_pos = self.get_loc()
    |           DateTimeToken(
771 |             OffsetDateTime("\{datetime_str}\{offset}"),
    |             ^^^^^^^^^^^^^^
    |             loc=make_loc(start_pos, end_pos),
    |           )
...
<WORKDIR>/toml_utils.mbt:652:18-652:32:
    |   set_dotted_key_value(
    |     table,
    |     ["root", "nested", "datetime"],
652 |     TomlDateTime(OffsetDateTime("2024-01-01T12:00:00Z")),
    |                  ^^^^^^^^^^^^^^
    |   )
    |   debug_inspect(
