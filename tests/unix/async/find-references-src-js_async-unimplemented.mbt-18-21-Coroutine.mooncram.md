...
Found 47 references for symbol 'Coroutine':
...
<WORKDIR>/src/internal/event_loop/event_loop.mbt:58:23-58:32:
   | priv struct QueuedJob {
   |   job_id : Int
   |   job : Job
58 |   waiter : @coroutine.Coroutine
   |                       ^^^^^^^^^
   | }
   | 
...
<WORKDIR>/src/internal/event_loop/signal.mbt:101:29-101:38:
    | #cfg(not(platform="windows"))
    | async fn EventLoop::terminate_signal_handler_unix(
    |   evloop : EventLoop,
101 |   sigwait_task : @coroutine.Coroutine,
    |                             ^^^^^^^^^
    | ) -> Unit {
    |   sigwait_task.cancel()
