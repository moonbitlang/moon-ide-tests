...
Found 379 references for symbol 'with_task_group':
...
<WORKDIR>/src/async.mbt:249:3-249:18:
    |     None
    |   }
    |   let results = Array::make(tasks.length(), None)
249 |   with_task_group() <| tg => {
    |   ^^^^^^^^^^^^^^^
    |     for i, task in tasks {
    |       if semaphore is Some(sem) {

<WORKDIR>/src/async.mbt:297:3-297:18:
    |   }
    |   let mut result = None
    |   let mut last_err = None
297 |   with_task_group() <| tg => {
    |   ^^^^^^^^^^^^^^^
    |     for task in tasks {
    |       if semaphore is Some(sem) {
...
<WORKDIR>/src/fs/watch_test.mbt:540:10-540:25:
    |   report_child_event~ : Bool,
    | ) -> Array[String] {
    |   let log = []
540 |   @async.with_task_group(group => {
    |          ^^^^^^^^^^^^^^^
    |     let test_dir = Dir({
    |       "outer": Dir({

<WORKDIR>/src/fs/watch_test.mbt:619:10-619:25:
    | ///|
    | async test "watch ignored path" {
    |   let log = []
619 |   @async.with_task_group(group => {
    |          ^^^^^^^^^^^^^^^
    |     let path = "_build/watch_ignored_path_test"
    |     let test_dir = Dir({

<WORKDIR>/src/fs/watch_test.mbt:768:10-768:25:
    |     "root_file": File("abcd"),
...
<WORKDIR>/src/internal/event_loop/worker_wbtest.mbt:167:12-167:27:
    | test "max worker limit" {
    |   let log = []
    |   with_event_loop(max_worker_count=2, () => {
167 |     @async.with_task_group <| group => {
    |            ^^^^^^^^^^^^^^^
    |       let start = @async.now()
    |       for i in 0..<4 {

<WORKDIR>/src/internal/event_loop/worker_wbtest.mbt:193:12-193:27:
    | test "cancel queued job" {
    |   let log = []
    |   with_event_loop(max_worker_count=1, () => {
193 |     @async.with_task_group <| group => {
    |            ^^^^^^^^^^^^^^^
    |       let start = @async.now()
    |       fn tick() {
...
<WORKDIR>/src/process/process.mbt:242:18-242:33:
    |     process.wait_pid(context~) catch {
    |       _ if @coroutine.is_being_cancelled() =>
    |         @async.protect_from_cancel() <| () => {
242 |           @async.with_task_group() <| group => {
    |                  ^^^^^^^^^^^^^^^
    |             group.spawn_bg(no_wait=true, () => cancel_handler(pid))
    |             process.wait_pid(context~)

<WORKDIR>/src/process/process.mbt:269:10-269:25:
    |   no_console_window? : Bool,
    | ) -> (Int, &@io.Data) {
    |   let (r, w) = read_from_process()
269 |   @async.with_task_group() <| group => {
    |          ^^^^^^^^^^^^^^^
    |     defer r.close()
    |     let exit_code = group.spawn(() => {

<WORKDIR>/src/process/process.mbt:305:10-305:25:
    |   no_console_window? : Bool,
    | ) -> (Int, &@io.Data) {
    |   let (r, w) = read_from_process()
305 |   @async.with_task_group() <| group => {
    |          ^^^^^^^^^^^^^^^
    |     defer r.close()
    |     let exit_code = group.spawn(() => {

<WORKDIR>/src/process/process.mbt:342:10-342:25:
    | ) -> (Int, &@io.Data, &@io.Data) {
    |   let (r_out, w_out) = read_from_process()
    |   let (r_err, w_err) = read_from_process()
342 |   @async.with_task_group() <| group => {
    |          ^^^^^^^^^^^^^^^
    |     let exit_code = group.spawn(() => {
    |       run(

<WORKDIR>/src/process/process.mbt:384:10-384:25:
    |   no_console_window? : Bool,
    | ) -> (Int, &@io.Data) {
    |   let (r, w) = read_from_process()
384 |   @async.with_task_group() <| group => {
    |          ^^^^^^^^^^^^^^^
    |     defer r.close()
    |     let exit_code = group.spawn(() => {
...
<WORKDIR>/src/process/wait_test.mbt:91:26-91:41:
   | #cfg(all(target="native", not(platform="windows")))
   | async test "signal exit code" {
   |   let sleep = sleep_prog.wait()
91 |   let exit_code = @async.with_task_group <| group => {
   |                          ^^^^^^^^^^^^^^^
