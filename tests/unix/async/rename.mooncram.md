...
```mooncram
$ run_moon_ide moon ide rename 'mkdir' 'mkdir_renamed' --loc 'src/fs/dir.mbt:24:14'
*** Begin Patch
...
```mooncram
$ run_moon_ide moon ide rename 'path' 'path_renamed' --loc 'src/fs/dir.mbt:25:3'
*** Begin Patch
*** Update File: <WORKDIR>/src/fs/dir.mbt
@@
 /// If `recursive=true` (`false` by default), non-existing parent directories
...
```mooncram
$ run_moon_ide moon ide rename 'File' 'FileRenamed' --loc 'src/fs/file.mbt:16:8'
*** Begin Patch
...
*** Update File: <WORKDIR>/src/tls/openssl.mbt
@@
 let _unused : Unit = {
   ignore(@os_error.check_errno)
   ignore(@os_string.encode)
-  ignore((_ : @fs.File) => ())
+  ignore((_ : @fs.FileRenamed) => ())
 }
*** End Patch
...
```mooncram
$ run_moon_ide moon ide rename 'io' 'io_renamed' --loc 'src/fs/file.mbt:17:3'
*** Begin Patch
*** Update File: <WORKDIR>/src/fs/file.mbt
@@
...
```mooncram
$ run_moon_ide moon ide rename 'kind' 'kind_renamed' --loc 'src/fs/utils.mbt:19:14'
*** Begin Patch
...
+    debug_inspect(@fs.kind_renamed(link_path), content="Regular")
...
```mooncram
$ run_moon_ide moon ide rename 'path' 'path_renamed' --loc 'src/fs/utils.mbt:19:19'
*** Begin Patch
*** Update File: <WORKDIR>/src/fs/utils.mbt
@@
 /// Get the kind of a file at `path`.
...
```mooncram
$ run_moon_ide moon ide rename 'FileIdentity' 'FileIdentityRenamed' --loc 'src/fs/watch.mbt:16:25'
*** Begin Patch
...
+  watched : Map[@fd_util.Fd, FileIdentityRenamed]
   watcher : Watcher
...
```mooncram
$ run_moon_ide moon ide rename 'WatchedFile' 'WatchedFileRenamed' --loc 'src/fs/watch.mbt:19:13'
*** Begin Patch
*** Update File: <WORKDIR>/src/fs/watch.mbt
@@
 using @event_loop {type FileIdentity}
...
```mooncram
$ run_moon_ide moon ide rename 'tmpdir_seed' 'tmpdir_seed_renamed' --loc 'src/fs/tmpdir.mbt:16:5'
*** Begin Patch
*** Update File: <WORKDIR>/src/fs/tmpdir.mbt
@@
 // limitations under the License.
...
```mooncram
$ run_moon_ide moon ide rename 'Rand' 'RandRenamed' --loc 'src/fs/tmpdir.mbt:16:22'
